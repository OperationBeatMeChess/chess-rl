...
class ThompsonSamplingTreeSearch:
    ...
    def __init__(self, game_state, nnet):
        ...
        self._alpha_states_actions = {}
        self._beta_states_actions = {}
    ...
    def search_iteration(self, game_state, game_obs, traversal_queue=None, c_param=1.4):
        ...
        if state not in self._legal_actions_states:
            ...
            with torch.no_grad():
                ...
                predicted_outcome = predicted_outcome.to('cpu')
        ...
        if (best_action, state) in self._alpha_states_actions:
            if result == 1: # assuming result is 1 for success and -1 for failure
                self._alpha_states_actions[(best_action, state)] += 1
            else:
                self._beta_states_actions[(best_action, state)] += 1
        else:
            self._alpha_states_actions[(best_action, state)] = 1 if result == 1 else 0.5
            self._beta_states_actions[(best_action, state)] = 1 if result == -1 else 0.5
    ...
    def best_action(self, state, c_param=1.4):
        ...
        for action in self._legal_actions_states[state]:
            alpha = self._alpha_states_actions.get((action, state), 1)
            beta = self._beta_states_actions.get((action, state), 1)

            # Draw a sample from the Beta distribution for this action
